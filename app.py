...
class IMApp(App):
        ...
        *,
        ...
        result = await execute_send_message(
            ...
            ensure_session_fn=lambda context, conv_id, target_peer_id: self._ensure_session(
                context, conv_id, target_peer_id
            ),
    ...
    # ------------------------------------------------------------------
    # Ensure session helper (fixes missing _ensure_session errors)
    # ------------------------------------------------------------------
    async def _ensure_session(self, context: SendMessageContext, conv_id: str, target_peer_id: str):
        # Delegate to the use-case helper that performs session establishment.
        # This wrapper ensures the method exists on IMApp and is awaitable.
        return await ensure_send_session(context, conv_id, target_peer_id)
    ...
    async def on_conversation_list_screen_open_friends(self, msg: ConversationListScreen.OpenFriends) -> None:
        from client.ui.screens.friends import FriendsScreen
        await self.push_screen(FriendsScreen())

    async def on_conversation_list_screen_request_reload(self, msg: ConversationListScreen.RequestReload) -> None:
        """App-level handler for ConversationListScreen.RequestReload messages."""
        try:
            await self.load_conversations()
        except Exception as exc:
            log.warning("request_reload_handler_failed", err=str(exc))
    ...
    async def on_friends_screen_send_request(self, msg: Any) -> None:
        ...
        if isinstance(result, FriendRequestSent):
            # After sending a request, refresh pending list and conversation list
            try:
                friends.handle_action_result(True, getattr(result, "status_message", "Request sent"), None)
            except Exception:
                # best-effort: trigger reload
                try:
                    await self.load_conversations()
                except Exception:
                    pass
    ...
    async def on_friends_screen_accept_request(self, msg: Any) -> None:
        ...
        # Update the friends screen UI (pending list + status)
        try:
            self._apply_friend_request_result(friends=friends, result=result)
        except Exception as exc:
            log.warning("apply_friend_request_result_failed", err=str(exc))

        # If accept succeeded, refresh conversations immediately (local) and notify conversation screens
        if isinstance(result, FriendRequestHandled):
            try:
                # Update local conversation store and repopulate any conversation screens
                await self.load_conversations()
            except Exception as exc:
                log.warning("load_conversations_after_accept_failed", err=str(exc))

            # Inform the friends screen that action succeeded (ensures UI buttons/state updated)
            try:
                friends.handle_action_result(True, getattr(result, "status_message", "Friend accepted"), msg.request_id)
            except Exception:
                # best-effort: post RequestReload so any ConversationListScreen will refresh
                try:
                    for s in list(getattr(self, "screen_stack", [])):
                        if isinstance(s, ConversationListScreen):
                            s.post_message(ConversationListScreen.RequestReload())
                except Exception:
                    pass
        else:
            # On failure, re-enable the buttons for retry and inform the screen
            try:
                friends.handle_action_result(False, getattr(result, "message", "Failed"), msg.request_id)
            except Exception:
                try:
                    friends.reenable_request_buttons(msg.request_id)
                except Exception as exc:
                    log.warning("reenable_buttons_failed", err=str(exc))
    ...
    async def on_friends_screen_decline_request(self, msg: Any) -> None:
        ...
        # On failure, re-enable the buttons for retry
    ...
    async def load_conversations(self) -> None:
        """Fetch updated conversations from backend and repopulate any ConversationListScreen.

        This updates conversation screens even when they are not the active screen
        (e.g., FriendsScreen is on top).
        """
        # 1) Sync with server to ensure latest conversation metadata is stored locally
        try:
            await self._sync_conversations_from_server()
        except Exception as exc:
            log.warning("sync_conversations_failed", err=str(exc))

        # 2) Read local conversations from store
        try:
            convs = await get_conversations()
        except Exception as exc:
            log.warning("get_conversations_failed", err=str(exc))
            convs = []

        # 3) Rehydrate peer username cache used for key lookups
        try:
            self._rehydrate_peer_usernames(convs)
        except Exception as exc:
            log.warning("rehydrate_peer_usernames_failed", err=str(exc))

        # 4) Build view models once
        try:
            summaries = self._build_conversation_summaries(convs)
        except Exception as exc:
            log.warning("build_conversation_summaries_failed", err=str(exc))
            summaries = []

        # 5) Repopulate any ConversationListScreen instances in the screen stack
        try:
            for s in list(getattr(self, "screen_stack", [])):
                if isinstance(s, ConversationListScreen):
                    try:
                        s.populate(summaries)
                    except Exception as exc:
                        log.warning("populate_conversation_screen_failed", err=str(exc))
        except Exception as exc:
            # Fallback: if screen_stack not available, try active screen
            screen = getattr(self, "screen", None)
            if isinstance(screen, ConversationListScreen):
                try:
                    screen.populate(summaries)
                except Exception as exc2:
                    log.warning("populate_active_conversation_screen_failed", err=str(exc2))

    # ------------------------------------------------------------------
    # WebSocket message handler (ensure this is an IMApp method)
    # ------------------------------------------------------------------

    async def _on_ws_message(self, msg: dict) -> None:
        """Top-level WebSocket message handler invoked by the login/context code.

        It dispatches incoming push messages to internal handlers.
        """
        ...
        # Server-side friend/contacts update (e.g., someone accepted a request)
        if msg_type in ("friend_update", "contacts_changed"):
            payload = msg.get("payload", {}) or {}
            try:
                # If server included conversation metadata, persist it locally first
                ...
                if conv:
                    try:
                        await upsert_conversation(
                            ...
                            peer_username=conv.get("peer_username"),
                            last_message_at=conv.get("last_message_at"),
                            unread_count=conv.get("unread_count", 0),
                        )
                    except Exception as exc:
                        log.warning("upsert_conversation_from_push_failed", err=str(exc))

                # Canonical refresh path: sync (best-effort) and repopulate conversation screens
                try:
                    await self.load_conversations()
                except Exception as exc:
                    log.warning("load_conversations_on_friend_update_failed", err=str(exc))
            except Exception as exc:
                log.warning("friend_update_handler_failed", err=str(exc))
            return

        # Unknown push type — log and ignore
        log.debug("ws_unknown_message_type", type=msg_type)
    ...
    async def _handle_incoming_message(self, payload: dict) -> None:
        ...
        try:
            await save_message(
                ...
                ttl_seconds=envelope.ttl_seconds if hasattr(envelope, "ttl_seconds") else None,
            )
        except Exception as exc:
            log.warning("save_incoming_message_failed", err=str(exc))
        ...
        try:
            ...
            log.warning("upsert_conversation_failed", err=str(exc))

        # If chat is active and matches conversation, render immediately
        if active_chat and active_chat.conversation_id == conv_id:
            active_chat.add_message(peer_id, plaintext, envelope.sent_at, "received", None, is_mine=False)
            ...
            # increment unread counter in store
            try:
                await increment_unread(conv_id)
            except Exception:
                pass

        # Notify conversation list screens to refresh
        try:
            await self.load_conversations()
        except Exception:
            pass
