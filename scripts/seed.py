...
DEFAULT_CA_CERT = Path("./certs/server.crt")
...
def _dev_tls_verify(server: str, *, insecure_local_tls: bool) -> bool | str:
    if not server.startswith("https://"):
        return True
    if insecure_local_tls:
        warn("TLS verification disabled for local seeding. Use only for local/demo fallback.")
        return False
    if DEFAULT_CA_CERT.exists():
        return str(DEFAULT_CA_CERT)
    return True
...
async def register_user(
    ...
    try:
        ...
        if "409" in str(exc) or "already" in str(exc).lower():
            ...
            if not totp_secret:
                error(
                    f"  No saved TOTP secret for {username!r} — cannot login. "
                    "Run with --clean to reset."
                )
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable TLS verification for local/demo fallback only. "
            "Preferred path uses ./certs/server.crt."
        ),
    )
    ...
    if args.clean:
        ...
        docker = _docker_bin()
        subprocess.run([docker, "compose", "down", "-v"], check=True)  # noqa: S603
        subprocess.run([docker, "compose", "up", "-d"], check=True)  # noqa: S603
    ...
    async with httpx.AsyncClient(
        base_url=server,
        verify=_dev_tls_verify(
            server,
            insecure_local_tls=args.insecure_local_tls,
        ),
        timeout=15.0,
    ) as client:
        # Health check
