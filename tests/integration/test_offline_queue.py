...
async def app_client():
    ...
    try:
        os.unlink("/tmp/test_offline.db")  # noqa: S108
...
async def test_offline_queue_store_and_forward(app_client):
    ...
    r = await app_client.post(
        "/v1/friends/request",
        json={"recipient_username": "off_bob"},
        headers=_auth(a_tok),
    )
    assert r.status_code == 201, r.text
    req_id = r.json()["id"]
    r = await app_client.put(
        f"/v1/friends/request/{req_id}", json={"action": "accept"}, headers=_auth(b_tok)
    )
    ...
    alice_sess, eph_pub, conv_dh_pub = derive_session_key_as_initiator(
        ...
        peer_identity_pub_bytes=bob_id_pub,
        peer_dh_pub_bytes=bob_dh_pub,
        my_user_id=alice_id,
        peer_user_id=bob_id,
    ...
    r = await app_client.post(
        "/v1/messages", json={"envelope": env.model_dump()}, headers=_auth(a_tok)
    )
    assert r.status_code == 201, r.text
...
async def test_offline_queue_replay_rejected(app_client):
    ...
    r = await app_client.post(
        "/v1/friends/request",
        json={"recipient_username": "roff_bob"},
        headers=_auth(a_tok),
    )
    req_id = r.json()["id"]
...
async def test_server_rejects_counter_regression(app_client):
    """Server rejects a lower counter even when the message ID is fresh."""
    pw = "CounterReg_p4ss!"
    alice_id_kp, alice_dh_kp, a_sec = await _register(app_client, "creg_alice", pw)
    bob_id_kp, bob_dh_kp, b_sec = await _register(app_client, "creg_bob", pw)
    a_tok = await _login(app_client, "creg_alice", pw, a_sec)
    b_tok = await _login(app_client, "creg_bob", pw, b_sec)

    r = await app_client.get("/v1/keys/creg_alice", headers=_auth(a_tok))
    alice_id = r.json()["user_id"]
    r = await app_client.get("/v1/keys/creg_bob", headers=_auth(a_tok))
    bob_id = r.json()["user_id"]
    bob_dh_pub = base64.b64decode(r.json()["dh_pub_b64"])
    bob_id_pub = base64.b64decode(r.json()["identity_pub_b64"])
    ...
    r = await app_client.post(
        "/v1/friends/request",
        json={"recipient_username": "creg_bob"},
        headers=_auth(a_tok),
    )
    req_id = r.json()["id"]
    await app_client.put(
    ...
    first_env = build_and_encrypt(
        send_chain=alice_send,
        plaintext="newest message",
        sender_id=alice_id,
        recipient_id=bob_id,
        conversation_id=conv_id,
        counter=1,
    ...
    first_env.eph_pub_b64 = base64.b64encode(eph_pub).decode()
    first_env.conv_dh_pub_b64 = base64.b64encode(conv_dh_pub).decode()

    first_resp = await app_client.post(
        "/v1/messages",
        json={"envelope": first_env.model_dump()},
        headers=_auth(a_tok),
    )
    assert first_resp.status_code == 201, first_resp.text

    regressed_env = build_and_encrypt(
        send_chain=alice_send,
        plaintext="older counter",
        sender_id=alice_id,
        recipient_id=bob_id,
        conversation_id=conv_id,
        counter=0,
    ...
    regressed_env.eph_pub_b64 = base64.b64encode(eph_pub).decode()
    regressed_env.conv_dh_pub_b64 = base64.b64encode(conv_dh_pub).decode()

    regressed_resp = await app_client.post(
        "/v1/messages",
        json={"envelope": regressed_env.model_dump()},
        headers=_auth(a_tok),
    )
    assert regressed_resp.status_code == 409, regressed_resp.text
    assert "counter" in regressed_resp.text.lower()
