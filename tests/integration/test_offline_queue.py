...
async def test_offline_queue_store_and_forward(app_client):
    ...
    r = await app_client.post(
        "/v1/friends/request", json={"recipient_username": "off_bob"}, headers=_auth(a_tok)
    )
    ...
    r = await app_client.get(
        "/v1/messages", params={"conversation_id": conv_id, "limit": 10}, headers=_auth(b_tok)
    )
    assert r.status_code == 200, r.text
...
async def test_offline_queue_replay_rejected(app_client):
    ...
    r = await app_client.post(
        "/v1/friends/request", json={"recipient_username": "roff_bob"}, headers=_auth(a_tok)
    )
    req_id = r.json()["id"]
